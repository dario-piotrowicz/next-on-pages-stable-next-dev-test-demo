...
async function getDOBindingInfo(durableObjects) {
  ...
  const registeredWorkersWithDOs = getRegisteredWorkersWithDOs(registeredWorkers);
      ...
      for (const [workerName, worker] of registeredWorkersWithDOs.entries()) {
        ...
        if (workerName === durableObject.service.name) {
          found = !!worker.durableObjects.find(
            (durableObject2) => durableObject2.className === durableObject2.className
          );
        }
        if (found)
          break;
...
function getRegisteredWorkersWithDOs(registeredWorkers) {
...
async function instantiateMiniflare(options) {
  ...
  const { workerOptions, durableObjects } = await getDOBindingInfo(
    devBindingsDurableObjectOptions
  ) ?? {};
...
function getPersistOption(localStoragePersistance) {
...
async function getMiniflareBindingOptions(bindings) {
  ...
  const {
    ...
    (acc, curr) => {
      const [bindingName, bindingDetails] = curr;
